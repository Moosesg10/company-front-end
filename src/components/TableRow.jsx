...
function TableRow({
  ...
  switch (sal.length) {
    case 5:
      newSalary = sal[0] + sal[1] + "." + sal[2] + sal[3] + sal[4];
      break;
    case 6:
      newSalary = sal[0] + sal[1] + sal[2] + "." + sal[3] + sal[4] + sal[5];
      break;
  }
  ...
  return (
    <tr>
      ...
      <td style={{ borderRight: "thin solid #f5f5f5" }}>
        <button onClick={() => delet(id, index, data, roles, deletEmployee)}>
          Eliminar
        </button>
        <button onClick={() => edit(id, index)}>Editar</button>
      </td>

      {roles === import.meta.env.VITE_ROL_Z && (
        <td>
          <select
            onChange={(e) => {
